...
public class StartupOrchestratorHostedService : IHostedService
{
    ...
    /// SQLite 下由 DatabaseInitializer 按 EF 模型建表（含新增列），跳过手写 PG DDL。
    /// </summary>
    private static void EnsureMedicalTables(FamilyDbContext db)
    {
        if (Baihua.Data.DbConnections.IsSqlite) return;
        const string ddl = """
    ...
    /// SQLite 下由 DatabaseInitializer 按 EF 模型建表，跳过手写 PG DDL。
    /// </summary>
